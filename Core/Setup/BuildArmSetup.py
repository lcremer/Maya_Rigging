...
def buildArmSetup(name,
    ...
    if stretch == 1:
        js.stretchNetwork(name, side, ('ik_' + shoulderJoint), ('ik_' + wristJoint), wristCtrl[0], stretchType, midLock, elbowCtrl[0])
    ...
    ikHandle = pc.ikHandle(name=(name + side + '_arm_ikhandle'), startJoint=IK[0], endEffector=IK[len(IK)-1], solver='ikRPsolver')
    ...
    if val < 0.00001:        
        ...
        if stretchAxis[0] == 'tx':
            ...
            pc.move(0, 0, 0.7, tempJoint[0], r=True, ls=True, wd=True)
    # create ik handle for ik joint chain
    ikHandleA = pc.ikHandle(name=(name + side + '_wristRot_ikhandle'), startJoint=IK[len(IK)-1], endEffector=tempJoint[0], solver='ikSCsolver')
    # parent ik handle into wrist control and lock the transforms
    ...
    pc.setAttr((charRigInfo + '.' + (side+'armRig')), (wristCtrl[0]+' '+elbowCtrl[0]+' '+switchCtrl[0]+' '+fkContrlList[0]), type='string')
    ...
    if world:
        if pc.attributeQuery('spineRig', n=charRigInfo, ex=True):
            ...
            bsss.buildSpaceSwitchSetup(wristCtrl[0], wristConGrp[0],[spineRigArray[1],spineRigArray[0],(name+'worldB_ctrl')], ['chest', 'root', 'world'], 1)
            bsss.buildSpaceSwitchSetup(elbowCtrl[0], elbowConGrp[0],[spineRigArray[1],spineRigArray[0],(name+'worldB_ctrl')], ['chest', 'root', 'world'], 1)
            bsss.buildSpaceSwitchSetup(fkContrlList[0], fkShoulderParent[0],[parentJoint[0],spineRigArray[1],(name+'worldB_ctrl')], ['clav', 'chest', 'world'], 2)
