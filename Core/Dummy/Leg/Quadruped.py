...
def build(name, side, num_joints, fingers, num_fingers, num_segment, color_index):
    ...
    hock_joint = create_module_annotation((name + side + 'hockJoint'), all_leg_dummy_joints[num_joints - 2])
    pc.setAttr((hock_joint + '.t'), (0, 1, 0))
    ...
    for a in all_leg_dummy_joints:
        joint_list = joint_list + (a + ' ')
    ...
    if fingers:
        ...
        finger_joint_list = ''
        finger_pos = [(1, 0, 1.5), (1.7, 0, 1.5), (2.4, 0, 1.5), (3.1, 0, 1.5), (3.8, 0, 1.5)]
        ...
        for i in range(num_fingers):
            ...
            pc.setAttr((finger + '.parent'), ankle_dummy_joint, type='string')
    ...
    pc.setAttr((main_leg_placer[0] + '.moduleTag'), 'quadLeg', type='string')
    ...
    pc.addAttr(main_leg_placer[0], ln=(side + 'hockJoint'), dt='string')
    pc.setAttr((main_leg_placer[0] + '.' + (side + 'hockJoint')), all_leg_dummy_joints[num_joints - 2], type='string')
    ...
    for i in range(0, len(all_leg_dummy_joints)):
        if i > 0:
            dU.add_dummy_twist_joints_attr(main_leg_placer[0], all_leg_dummy_joints[i - 1], all_leg_dummy_joints[i], 2)
    dU.add_dummy_twist_joints_attr(main_leg_placer[0], all_leg_dummy_joints[len(all_leg_dummy_joints) - 1],
                                   ankle_dummy_joint[0],
                                   2)
    dU.add_dummy_twist_joints_attr(main_leg_placer[0], ankle_dummy_joint[0], ball_dummy_joint[0], 0)
