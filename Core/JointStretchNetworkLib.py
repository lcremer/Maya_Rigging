...
def stretchNetwork(name, side, startJoint, endJoint, controller, type, midLock, midController):
    ...
    defaultLength = pc.getAttr(dist[3] + '.output')
    ...
    pc.connectAttr((dist[3] + '.output'), (ssMDN + '.input1X'), f=True)
    ...
    pc.connectAttr((condASN + '.offset'), (condASN + '.input1D[0]'),
    ...
    offInvMDN = pc.shadingNode('multiplyDivide', n=(name + side + 'offsetPreInv_md'),
    ...
    offPreMDN = pc.shadingNode('multiplyDivide', n=(name + side + 'offsetPre_md'),
                               asUtility=True)  # used to adjust the scale value for pre middle joints
    offPostMDN = pc.shadingNode('multiplyDivide', n=(name + side + 'offsetPost_md'),
                                asUtility=True)  # used to adjust the scale for post middle joints
    offPreASN = pc.shadingNode('plusMinusAverage', n=(name + side + 'offsetPre_pma'),
                               asUtility=True)  # used to offset the offset controls
    pc.addAttr(offPreASN, ln='offset', at='double',
               dv=1)  # add an attibute with a value of 1 to set the default scale value
    offPostASN = pc.shadingNode('plusMinusAverage', n=(name + side + 'offsetPost_pma'),
                                asUtility=True)  # used to offset the offset controls
    pc.addAttr(offPostASN, ln='offset', at='double',
               dv=1)  # add an attibute with a value of 1 to set the default scale value
    ...
    if type == 'scale':
        if midLock == 1:
            ...
            preMid = chUL.createDistance(list[0], midController, True)
            ...
            pc.connectAttr((preMid[3] + '.output'), (preMidScaler_MDN + '.input1X'), f=True)
            ...
            # create distance node from start to mid
            postMid = chUL.createDistance(midController, controller, True)
            ...
            # do all necessary connection for mid lock setup
            pc.connectAttr((postMid[3] + '.output'), (postMidScaler_MDN + '.input1X'), f=True)
...
def buildIkStretch(name, side, startJoint, endJoint, controller, stretchType):
    ...
    dist = chUL.createDistance(startJoint, endJoint, True)
    ...
    ikHandle = pc.ikHandle(name=(name + side + startJoint + '_ikhandle'), startJoint=startJoint, endEffector=endJoint,
    ...
    pc.connectAttr((dist[3] + '.output'), (scalerMD + '.input1X'))
