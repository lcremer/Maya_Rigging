...
def createDistance(start_joint, end_joint, add_conversion=False):
    ...
    if add_conversion:
        conversion_node = pc.createNode('unitConversion')
        pc.setAttr(conversion_node + '.conversionFactor', Units.conversion_factor())
        result.append(conversion_node)
        pc.connectAttr(result[0]+".distance", result[3]+".input")
    return result
...
def makeFootHeelPivots(ankleJoint, ballJoint):
    ...
    tempCon = pc.aimConstraint(endJoint, tempLoc[0], o=[0, 0, 0], w=1, aimVector=[0, 0, 1], upVector=[0, 1, 0],
...
def getQuadFlexLegJointPos(startJoint, endJoint):
    ...
    animCon = pc.aimConstraint(endJoint, startLoc[0], offset=[0, 0, 0], weight=1, aimVector=[0, -1, 0],
                               upVector=[0, 0, 1], worldUpType='vector', worldUpVector=[0, 1, 0])
    pc.delete(animCon)
